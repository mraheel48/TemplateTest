...
public class SVGParser {
    ...
    private static void drawArc(Path p, float lastX, float lastY, float x, float y, float rx, float ry, float theta, int largeArc, int sweepArc) {
        ...
        final Double thrad = theta * (double) Math.PI / 180;
        ...
        double lambda = (x1ts / rxs + y1ts / rys) * 1.001f; // add 0.1% to be
        // sure that no out
        // of range occurs
        // due to
        // limited precision
        ...
        final double R = Math.sqrt((rxs * rys - rxs * y1ts - rys * x1ts) / (rxs * y1ts + rys * x1ts)) * ((largeArc == sweepArc) ? -1 : 1);
        ...
        final double th1 = angle(1, 0, (float) (x1t - cxt) / rx, (float) (y1t - cyt) / ry);
        double dth = angle((float) (x1t - cxt) / rx, (float) (y1t - cyt) / ry, (float) (-x1t - cxt) / rx, (float) (-y1t - cyt) / ry);
                ...
                if (viewboxStr != null) {
                    ...
                    if (dims.length == 4) {
                        ...
                        if (x1 != null && x2 != null && y1 != null && y2 != null) {
                            double width = Math.ceil(x2 - x1);
                            double height = Math.ceil(y2 - y1);
                            canvas = picture.beginRecording((int) width, (int) height);
                            canvasRestoreCount = canvas.save();
                            canvas.clipRect(0f, 0f, (float) width, (float) height);
